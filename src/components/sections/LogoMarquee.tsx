...
function UniqueMarqueeRow({ items }: { items: LogoMarqueeItem[] }) {
  ...
  useEffect(() => {
    ...
    const rails = wrap.closest(".logo-marquee-rails");
    const pause = () => {
    ...
    rails?.addEventListener("mouseenter", pause);
    rails?.addEventListener("mouseleave", resume);
    ...
    const tick = (now: number) => {
      ...
      if (!pausedRef.current && track.scrollWidth > wrap.clientWidth) {
    ...
    return () => {
      ...
      rails?.removeEventListener("mouseenter", pause);
      rails?.removeEventListener("mouseleave", resume);
    };
  }, []);
  ...
  return (
    <div ref={wrapRef} className="overflow-hidden">
      <div
        ref={trackRef}
        className="flex w-max items-start gap-3 px-2 will-change-transform sm:gap-5"
      >
        {order.map((item) => (
          <LogoSeal
            key={item.name}
            ...
            logo={item.logo}
            bg={item.bg}
            index={items.findIndex((brand) => brand.name === item.name) + 1}
          />
        ))}
      </div>
    </div>
    ...
    <section className="relative overflow-hidden bg-brand py-24 text-white md:py-32">
      ...
      <div className="logo-marquee-rails relative mt-16 md:mt-20">
        ...
        <div
          ...
          aria-hidden
        />
        {loop ? <MarqueeRow items={items} /> : <UniqueMarqueeRow items={items} />}
      </div>
    </section>
  );
}
